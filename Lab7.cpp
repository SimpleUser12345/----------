...
template<class T> void Creatematr(T*** matr,int n,int m){
        T z;z=0;
        if (n) {
                (*matr)=new T*[n];
                for(int i=0;i<n;i++){
...
template<class T> void Destroymatr(T** matr,int n,int m){
        for(int i=0;i<n;i++){
                delete[] matr[i];
        }
        delete[] matr;
...
template<class T> void Createvect(T** vect,int n){
        T z;z=0;
        (*vect)=new T[n];
        for(int i=0;i<n;i++){
                (*vect)[i]=z;
...
template<class T,class Q> void Copyvect(T* src,Q* cp,int n){
        for(int i=0;i<n;i++){
                cp[i]=src[i];
        }
}
...
private:
	...
	void Build(){
		...
		while (FCTime<FNTime+FCurrentStep&&(FS<FN-2)){
				...
				SearchPeice();
				CalcF(FMemory[FS+1],FDiff[FS+1], 0);
		}
		delete [] CPiece;
		delete [] Delta;
	...
	void SearchPeice(void){
		...
		TMyReal CX=FCTime+FCurrentStep;
